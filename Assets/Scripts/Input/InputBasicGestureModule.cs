...
public class InputBasicGestureModule : MonoBehaviour
{
    ...
    private void PerformPinch()
    {
        ...
        float distance = Vector2.Distance(InputTouch.PercentPositionPixel(touch1Pos), InputTouch.PercentPositionPixel(touch2Pos));
        float oldDistance = Vector2.Distance(InputTouch.PercentPositionPixel(touch1OldPos), InputTouch.PercentPositionPixel(touch2OldPos));
    ...
    private void PerformHoldDrag()
    {
        ...
        else if (InputTouch.TouchCount == 2)
        {
            ...
            if (touch1.phase == TouchPhase.Moved && touch2.phase == TouchPhase.Moved)
                OnPan?.Invoke(InputTouch.PercentPositionPixel((touch1.deltaPosition + touch2.deltaPosition) * 0.5f));
        }
    }
}
